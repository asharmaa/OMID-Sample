...
class BaseAdUnitViewController: UIViewController {
    ...
    var adSession: OMIDAppnexusAdSession?
    var adEvents: OMIDAppnexusAdEvents?
...
//        self.dismissAd()
    ...
    */
    ...
    func startMeasurement() {
        ...
        do {
            ...
            adEvents = try OMIDAppnexusAdEvents(adSession: adSession)
        ...
        do {
            ...
            if adSession?.configuration.impressionOwner == .nativeOwner {
                ...
                // No need to fire adEvents if HTMLVideo adUnit as the Javascript does this
                return
            }
    ...
    */
    ...
    private func activateOMSDK() -> Bool {
        if OMIDAppnexusSDK.shared.isActive {
            return true
        }

        // Activate the SDK
        OMIDAppnexusSDK.shared.activate()
       
        return OMIDAppnexusSDK.shared.isActive

    }

    private func createAdSession() -> OMIDAppnexusAdSession {
        ...
        guard let partner = OMIDAppnexusPartner(name: partnerName, versionString: partnerVersion ?? "1.0")
            else {
        ...
        do {
            // Create ad session
            let session = try OMIDAppnexusAdSession(configuration: configuration, adSessionContext: context)
...
//            try session.addFriendlyObstruction(closeButton,
//                                               purpose: .closeAd,
//                                               detailedReason: "Close Ad Button")
     ...
     */
    func setupAdditionalAdEvents(adSession: OMIDAppnexusAdSession) {
        return
    }
     ...
     */
    func createAdSessionContext(withPartner partner: OMIDAppnexusPartner) -> OMIDAppnexusAdSessionContext {
        fatalError("Not implemented")
    }
     ...
     */
    func createAdSessionConfiguration() -> OMIDAppnexusAdSessionConfiguration {
        fatalError("Not implemented")
    }
    
    func createVerificationScriptResource(vendorKey: String?, verificationScriptURL: String, parameters: String?) -> OMIDAppnexusVerificationScriptResource? {
        ...
        if let vendorKey = vendorKey,
            ...
            vendorKey.count > 0 && parameters.count > 0 {
                return OMIDAppnexusVerificationScriptResource(url: URL,
                                                               vendorKey: vendorKey,
...
extension BaseAdUnitViewController {
    ...
    func showErrorMessage(message: String) {
        if isDisplayingErrorMessage == false {
            ...
            let alert = UIAlertController(title: "An Error Was Encountered", message: message, preferredStyle: UIAlertController.Style.alert)
